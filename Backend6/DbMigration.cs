...
namespace Backend6
{
    public static class DbMigration
    {
        ...
        private static async Task ConfigureIdentity(IServiceScope scope)
        {
            ...
            var moderRole = await roleManager.FindByNameAsync(ApplicationRoles.Moderators);
            if (moderRole == null)
            {
                ...
                moderRole = await roleManager.FindByNameAsync(ApplicationRoles.Moderators);
            }

            var moderUser = await userManager.FindByNameAsync("moderator@localhost.local");
            if (moderUser == null)
            {
                var userResult = await userManager.CreateAsync(new ApplicationUser
                {
                    UserName = "moderator@localhost.local",
                    Email = "moderator@localhost.local"
                }, "AdminPass123!");
                if (!userResult.Succeeded)
                {
                    throw new InvalidOperationException($"Unable to create admin@localhost.local user");
                ...
                moderUser = await userManager.FindByNameAsync("moderator@localhost.local");
            }

            if (!await userManager.IsInRoleAsync(moderUser, moderRole.Name))
            {
                await userManager.AddToRoleAsync(moderUser, moderRole.Name);
            }
        }
    }
}
